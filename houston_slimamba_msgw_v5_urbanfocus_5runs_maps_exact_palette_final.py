...
class SlimWaveletHoustonNet(nn.Module):
    def __init__(
        ...
        self.graph = GraphWaveletFusion(base_ch * 2 + lidar_ch, graph_ch, patch_size, scales=[0.7, 1.4, 2.8, 4.0])
        self.urban_strip = UrbanStripMixer(base_ch * 2 + lidar_ch + graph_ch, urban_ch, drop_path=drop_path)
        self.center_mlp = nn.Sequential(
            nn.Linear(base_ch * 2 + lidar_ch, base_ch),
            nn.GELU(),
...
def save_map_like_demo(img2d: np.ndarray, title: str, path: str, colors: Sequence[str] = colors, class_names: Sequence[str] = categories) -> None:
    fig, ax = plt.subplots(figsize=(18, 4.8))
    ax.imshow(img2d, cmap=cmap, vmin=0, vmax=len(colors))
    ax.set_title(title, fontsize=16)
    ax.axis("off")
    handles = [Patch(facecolor=colors[i], edgecolor="k", label=class_names[i]) for i in range(len(class_names) - 1, -1, -1)]
...
def save_map_pair(
    ...
    fig, axes = plt.subplots(2, 1, figsize=(16, 7.8))
    axes[0].imshow(left_img, cmap=cmap, vmin=0, vmax=len(colors))
    axes[0].set_title(left_title, fontsize=14)
    axes[0].axis("off")
    axes[1].imshow(right_img, cmap=cmap, vmin=0, vmax=len(colors))
    axes[1].set_title(right_title, fontsize=14)
    axes[1].axis("off")
    handles = [Patch(facecolor=colors[i], edgecolor="k", label=class_names[i]) for i in range(len(class_names) - 1, -1, -1)]
    labels = [class_names[i] for i in range(len(class_names) - 1, -1, -1)]
    axes[1].legend(handles, labels, loc="center left", bbox_to_anchor=(1.01, 0.5), frameon=False, fontsize=9)
    plt.tight_layout()
    fig.savefig(path, bbox_inches="tight", dpi=300)
    plt.close(fig)
...
def save_run_artifacts(
    ...
    save_map_like_demo(gt.astype(np.uint8), "Ground Truth Land-cover Map (All Labeled Pixels)", gt_path, CLASS_COLORS, CLASS_NAMES)
    save_map_like_demo(ts_label.astype(np.uint8), "Ground Truth Land-cover Map (Official Test Pixels)", gt_test_path, CLASS_COLORS, CLASS_NAMES)
    save_map_like_demo(pred_full_img, "Predicted Land-cover Classification Map (All Labeled Pixels)", pred_full_path, CLASS_COLORS, CLASS_NAMES)
    save_map_like_demo(pred_test_img, "Predicted Land-cover Classification Map (Official Test Pixels)", pred_test_path, CLASS_COLORS, CLASS_NAMES)
    save_map_pair(gt.astype(np.uint8), pred_full_img, "Ground Truth Land-cover Map (All Labeled Pixels)", "Predicted Land-cover Classification Map (All Labeled Pixels)", pair_full_path, CLASS_COLORS, CLASS_NAMES)
    save_map_pair(ts_label.astype(np.uint8), pred_test_img, "Ground Truth Land-cover Map (Official Test Pixels)", "Predicted Land-cover Classification Map (Official Test Pixels)", pair_test_path, CLASS_COLORS, CLASS_NAMES)
    plot_per_class_bar(per_class, CLASS_NAMES[: len(per_class)], per_class_path, "Land-cover Classification Accuracy", CLASS_COLORS[: len(per_class)])
    ...
    with open(per_class_csv, "w", encoding="utf-8") as f:
        ...
        for i, acc in enumerate(per_class):
            f.write(f"{i + 1},{CLASS_NAMES[i]},{acc:.6f}\n")
...
def main() -> None:
    parser = argparse.ArgumentParser("SliMamba + multi-scale graph wavelet for Houston 2013, 5-run reporting with classification maps")
    parser.add_argument("--data-root", type=str, default="E:/PythonProject1/Houston/")
    parser.add_argument("--output-dir", type=str, default="houston_v5_urbanfocus_runs")
    ...
    print(f"[INFO] total train labels={int((tr_label > 0).sum())}, total test labels={int((ts_label > 0).sum())}")
    print(f"[INFO] device={device}")
    print(f"[INFO] protocol: runs={args.runs}, spatial-CV folds={args.cv_folds}, full-train ensemble seeds={args.final_seeds}")

    run_results: List[Dict] = []
    for run_id in range(1, args.runs + 1):
        base_seed = args.seed + 10000 * (run_id - 1)
        set_seed(base_seed)
        run_res = run_single_protocol(args, data, device, run_id, base_seed)
        run_results.append(run_res)

    summary = summarize_runs(run_results, os.path.join(args.output_dir, "summary"))

    print("\n" + "#" * 88)
    print(f"Final summary across {args.runs} runs")
    print(f"OA     : {summary['oa'].mean():.4f} ± {summary['oa'].std(ddof=0):.4f}")
    print(f"AA     : {summary['aa'].mean():.4f} ± {summary['aa'].std(ddof=0):.4f}")
    print(f"Kappa  : {summary['kappa'].mean():.4f} ± {summary['kappa'].std(ddof=0):.4f}")
    print("Per-class accuracy mean ± std")
    for i in range(len(summary["mean_pc"])):
        print(f"  {i + 1:02d}. {CLASS_NAMES[i]:<18s} : {summary['mean_pc'][i]:.4f} ± {summary['std_pc'][i]:.4f}")
